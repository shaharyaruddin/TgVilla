...
const BookingCard = ({ villa }) => {
  ...
  return (
    <div className="relative   my-20 max-xl:my-3 max-2xl:my-10 w-full min-h-[35rem] max-2xl:min-h-[30rem] max-xl:min-h-[28rem] max-md:min-h-[20rem] grid grid-cols-2 max-md:grid-cols-1 text-white">
      {/* Left - Image */}
      ...
      {/* Right - Details */}
      <div className="flex max-md:rounded-bl-xl max-md:rounded-br-xl flex-col max-md:sticky space-y-5 max-md:space-y-3 justify-center p-5 bg-[#614E3F] absolute left-1/2 -translate-x-[10%] top-1/2 -translate-y-[40%] min-w-[45vw] max-2xl:min-w-[54vw] max-md:min-w-full max-md:-translate-y-0 max-md:-translate-x-0">
        {/* Top */}
        <div className="w-full font-cormorant pb-3 max-xl:pb-2 flex flex-col border-b-2 border-white/40 text-2xl">
          <h3 className="text-lg max-xl:text-base">{villa.type}</h3>
          <h2 className="font-[700] max-xl:text-lg">{villa.name}</h2>
        </div>
        {/* Center */}
        <div className="flex justify-between items-center border-b-2 border-white/40 pb-6 max-xl:pb-3 max-md:flex-col max-md:items-start">
          ...
          <div className="flex max-md:justify-between max-md:w-full">
            ...
            <div className="flex items-center ml-4">
              <a
                href="#"
                className="border border-white px-4 py-2 items-center rounded-lg max-xl:text-sm"
              >
                RESERVE
              </a>
            </div>
          ...
          <div className="py-2 max-xl:py-1">
            <ul>
              {villa.services.map((service, index) => (
                <li
                  key={index}
                  className="flex items-center max-xl:text-sm space-x-2 cursor-pointer"
                >
